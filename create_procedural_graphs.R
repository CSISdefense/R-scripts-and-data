...
create_procedural_graphs<-function(data.name,section.variable.name,start.year=NULL,LayoutName=NA){
    ...
    #Treat Newlines as Newlines rather than as \n.
    choice.figures$figure.title <- gsub("\\\\n","\n",choice.figures$figure.title)
    choice.figures$x.axis.title <- gsub("\\\\n","\n",choice.figures$x.axis.title)
    choice.figures$y.axis.title <- gsub("\\\\n","\n",choice.figures$y.axis.title)
    
    if(all(is.numeric(choice.figures$figure.number))){
    ...
    if(is.na(LayoutName)){
        ...
        which.layout<-choice.layout$Layout.Name==LayoutName
    }
    #         axis.text.size<-0.65
