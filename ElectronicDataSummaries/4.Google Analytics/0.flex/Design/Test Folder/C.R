...
p <- switch(input$cat, 
              ...
              Type = {ggplot(data = dataset(), aes(x = Type, y = Views)) +
                  coord_cartesian(ylim = c(0,max(GAdata$Views)))},
              Facebook = {ggplot(data = dataset(), aes(x = Facebooked,
                                                       y = Views)) +
                  coord_cartesian(ylim = c(0,max(GAdata$Views)))}
...
p <- p + geom_boxplot(outlier.shape = NA, alpha = .1, color = "white") 
  
p <- switch(input$include,
              'All' = {
                p +
                  geom_point(aes(color = fromISP, size = fromISP, shape = fromISP),
                             alpha = 0.7) +
...
# runs the on-hover text output
output$hover_info <- renderPrint({
  if(!is.null(input$plot_hover)){
    hover <- input$plot_hover
    hovdata <- dataset()
    whichvar <- switch(input$cat,
                   Weekday = levels(hovdata$Weekday)[max(round(hover$x),1)], 
                   Block = levels(hovdata$Block)[max(round(hover$x),1)], 
                   Type = levels(hovdata$Type)[max(round(hover$x),1)],
                   Facebook = levels(hovdata$Facebooked)[max(round(hover$x),1)]
    )
    hovdata <- switch(input$cat,
                       Weekday = filter(hovdata, Weekday == whichvar), 
                       Block = filter(hovdata, Block == whichvar), 
                       Type = filter(hovdata, Type == whichvar),
                       Facebook = filter(hovdata, Facebooked == whichvar)
    )
    
    
    dist <- sqrt((hovdata$Views - hover$y)^2)
    
    cat("Report:\n")
    if(min(dist) < 123 & abs(hover$x - round(hover$x)) < 0.25)
      hovdata$Title[which.min(dist)]
  }
})


# runs the on-click text output
output$click_info <- renderUI({
  if(!is.null(input$plot_click)){
    click <- input$plot_click
    clickdata <- dataset()
    clickvar <- switch(input$cat,
                       Weekday = levels(clickdata$Weekday)[round(click$x)], 
                       Block = levels(clickdata$Block)[round(click$x)], 
                       Type = levels(clickdata$Type)[round(click$x)],
                       Facebook = levels(clickdata$Facebooked)[round(click$x)]
    )
    clickdata <- switch(input$cat,
                      Weekday = filter(clickdata, Weekday == clickvar), 
                      Block = filter(clickdata, Block == clickvar), 
                      Type = filter(clickdata, Type == clickvar),
                      Facebook = filter(clickdata, Facebooked == clickvar)
    )
    
    
    dist <- sqrt((clickdata$Views - click$y)^2)
    
    cat("Report:\n")
    if(min(dist) < 123 & abs(click$x - round(click$x)) < 0.25)
